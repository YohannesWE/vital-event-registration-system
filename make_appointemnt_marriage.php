...
function scheduleAppointments($username) {
    ...
    // Check if the user already has an appointment for the 'marriage ' event type after 3 days from today
    if (hasExistingAppointment($username, 'marriage ')) {
        $message = "An appointment for the 'marriage ' event type is already scheduled for $username after 3 days from today.";
        return $message;
    }

    // Check if the user already has an appointment for the 'marriage ' event type in the future
    $sql = "SELECT appointment_date FROM appointment WHERE username = '$username' AND event_type = 'marriage ' AND appointment_date > '$currentDate' ORDER BY appointment_date ASC LIMIT 1";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);

    if ($row) {
        $appointmentDate = $row['appointment_date'];
        $message = "An appointment for the 'marriage ' event type is already scheduled for $username on $appointmentDate.";
        return $message;
    }
    ...
    while (true) {
        ...
        if ($numAppointmentsToday < 10) {
            ...
            $insertSql = "INSERT INTO appointment (username, k_id_no, appointment_date, event_type)
                          VALUES ('$username', '$k_id_no', '$appointmentDate', '$marriage')";
...
// Display the message
header("Location: appointment_marriage.php?message=" . urlencode($message));
