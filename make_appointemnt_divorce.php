...
function scheduleAppointments($username) {
    ...
    if (hasExistingAppointment($username, 'divorce')) {
        $message = "An appointment for the 'divorce' event type is already scheduled for $username after 3 days from today.";
        return $message;
    }

    // Check if the user already has an appointment for the 'birth' event type in the future
    $sql = "SELECT appointment_date FROM appointment WHERE username = '$username' AND event_type = 'divorce' AND appointment_date > '$currentDate' ORDER BY appointment_date ASC LIMIT 1";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);

    if ($row) {
        $appointmentDate = $row['appointment_date'];
        $message = "An appointment for the 'divorce' is already scheduled for $username on $appointmentDate.";
        return $message;
    }
    ...
    while (true) {
        ...
        if ($numAppointmentsToday < 10) {
            ...
            $insertSql = "INSERT INTO appointment (username, appointment_date, event_type) 
                          VALUES ('$username', '$appointmentDate', '$divorce')";
...
// Display the message
header("Location: appointment_divorce.php?message=" . urlencode($message));
