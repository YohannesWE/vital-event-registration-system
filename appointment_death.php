...
$sql = "SELECT * FROM death_table WHERE Death_states = 'approved' and Payemnt ='paid'";
        ...
        <div class="table-container">
            ...
            <table>
                ...
                <?php if (mysqli_num_rows($result) == 0): ?>
                    <tr>
                        <td colspan="5"><?= __('No Ready unscheduled  Death certificate found')?>.</td>
                    </tr>
                <?php else: ?>
                    <?php while ($info = $result->fetch_assoc()): ?>
                        <tr>
                            <td><?php echo $info['d_id']; ?></td>
                            <td class="td_tale"><?php echo "{$info['f_name']} {$info['m_name']} {$info['l_name']}"; ?></td>
                            <td><?php echo $info['k_id_no']; ?></td>
                            <td><?php echo $info['Death_states']; ?></td>
                            <td>
                            <a href="make appointemnt death.php?k_id_no=<?php echo $info['k_id_no']; ?>" style="display: inline-block; padding: 10px 20px; background-color: green; color: white; text-decoration: none; border-radius: 5px;"><?= __('Make Appointment')?></a>
