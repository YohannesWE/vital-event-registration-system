...
// Report generation
$totalDeaths = $totalPending = $totalApproved = $totalPaid = $totalUnpaid = $totalCertified = 0;

// Total number of deaths
$totalDeathsQuery = "SELECT COUNT(*) AS total_deaths FROM death_table";
$totalDeathsResult = mysqli_query($conn, $totalDeathsQuery);
if ($totalDeathsResult && mysqli_num_rows($totalDeathsResult) > 0) {
    $totalDeathsRow = mysqli_fetch_assoc($totalDeathsResult);
    $totalDeaths = $totalDeathsRow['total_deaths'];
}

// Total number of pending deaths
$totalPendingQuery = "SELECT COUNT(*) AS total_pending FROM death_table WHERE Death_states = 'pending'";
...
// Total number of approved deaths
$totalApprovedQuery = "SELECT COUNT(*) AS total_approved FROM death_table WHERE Death_states = 'approved'";
...
// Total number of paid deaths
$totalPaidQuery = "SELECT COUNT(*) AS total_paid FROM death_table WHERE Payemnt = 'paid'";
...
// Total number of unpaid deaths
$totalUnpaidQuery = "SELECT COUNT(*) AS total_unpaid FROM death_table WHERE Payemnt = 'unpaid'";
...
// Total number of certified deaths (both approved and paid)
$totalCertifiedQuery = "SELECT COUNT(*) AS total_certified FROM death_table WHERE Death_states = 'approved' AND Payemnt = 'paid'";
...
<h5><center><?= __('Death Report')?></center></h5>
    

    <table border="1">
        <tr>
            <th><?= __('Total Number of Deaths')?></th>
            <td><?php echo $totalDeaths; ?></td>
        </tr>
        <tr>
            <th><?= __('Total Number of Pending Deaths')?></th>
            <td><?php echo $totalPending; ?></td>
        </tr>
        <tr>
            <th><?= __('Total Number of Approved Deaths')?></th>
            <td><?php echo $totalApproved; ?></td>
        </tr>
        <tr>
            <th><?= __('Total Number of Paid Deaths')?></th>
            <td><?php echo $totalPaid; ?></td>
        </tr>
        <tr>
            <th><?= __('Total Number of Unpaid Deaths')?></th>
            <td><?php echo $totalUnpaid; ?></td>
        </tr>
        <tr>
            <th><?= __('Total Number of Certified Deaths')?></th>
