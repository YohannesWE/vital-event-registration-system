...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["reply_message"]) && !empty(trim($_POST["reply_message"]))) {
        ...
        $c_id = $_POST["c_id"]; // Assuming f_id is the ID of the feedback being replied to

        // Prepare and execute the SQL query to update the feedback with the reply message
  // Prepare and execute the SQL query to update the feedback with the reply message
$sql = "UPDATE comment_table SET replay = ? WHERE c_id = ?";
$stmt = mysqli_prepare($conn, $sql);
if ($stmt) {
    mysqli_stmt_bind_param($stmt, "si", $replyMessage, $c_id);
...
<body>
    <div class="container">
        <h5><center>Reply to Feedback</center></h5>
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <input type="hidden" name="c_id" value="<?php echo isset($_GET['c_id']) ? htmlspecialchars($_GET['c_id']) : ''; ?>">
