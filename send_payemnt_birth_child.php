...
if (mysqli_num_rows($payment_check_result) > 0) {
    ...
    $message = "You have already made a payemnt.";
} else {
        ...
        if (mysqli_num_rows($birth_status_result) > 0) {
            ...
            if (isset($birth_status_row['Birth_status'])) {
                ...
                if ($birth_status == 'pending') {
                    $message = "You are not eligible for payment because your birth application under process.";
            ...
            $message = "No birth application found for the child.";
        }
    } else {
...
<h5><center><?= __('Send Payment Form')?></center></h5>
    ...
    <?php else: ?>
        ...
        <p><?= __('Check payment amounts')?> <a href="payment_info_child.php"><?= __('here')?> </a><?= __('before making payment')?> .</p>
